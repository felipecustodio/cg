...
GLfloat raquetePy = 215, raqueteIAPy = 250;
...
void bola(int passo)
{
	...
	if (direcaox == DIREITA) {
		movebolax += passo;
		// Se alcancou a raquete do usu�rio e este defendeu, muda de direcao, sen�o acabou o jogo.
		if (movebolax == 290) {
			float dist = abs(raquetePy - (movebolay + 210));
			if (dist <= 25)
				direcaox = ESQUERDA;
			else {
				// Desliga este timer, n�o deixando a bolinha andar mais.
				passo = 0;
				// Telona vermelha = PERDEU!!!
				glClearColor(1.0, 0.0, 0.0, 0.0);
				glClear(GL_COLOR_BUFFER_BIT);

				// N�o deixe mais o usu�rio movimentar a raquete.
	...
	if (direcaoy == CIMA) { // Bolinha subindo.
		...
		movebolay -= passo;
		// Se alcan�ou a borda de baixo, pode come�ar a subir
